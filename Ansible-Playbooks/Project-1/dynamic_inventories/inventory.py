...
class Inventory(object):
    ...
    def __init__(self, include_hostvars_in_list):
        ...
        # Store True/False depending on whether/not the parameter passed
        parser.add_argument('--list', action='store_true',
                            help='list inventory')
        # Store the value passed with `--host`
    ...
    def define_inventory(self):
        """Group of hosts with dictionary for groupvars inside
        """
        self.groups = {
            "centos": {
    ...
    def print_json(self, content):
        ...
        Args:
            content (json): json input
        """
        print(json.dumps(content, indent=4, sort_keys=True))

    # Return inventory dictionary
    def list(self):
        """lists the group info with/without hostvars

        Returns:
            dict: group info dict
        """
    ...
    def host(self):
        """Pass back the host context dict.
